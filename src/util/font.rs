...
lazy_static! {
    ...
    // static ref FONT_MAP: HashMap<str, Font<'static, 'static>> = HashMap::new();
    pub static ref DEFAULT_FONT: Arc<Mutex<UnsafeSendSync<Font<'static, 'static>>>> =
        Arc::new(Mutex::new(UnsafeSendSync::new(load_font(
            DEFAULT_FONT_NAME,
            DEFAULT_FONT_SIZE
        ))));
}
