        ...
        for body_ptr in B2world::get_body_list(&RefCell::borrow(&box2d.world_ptr)).iter() {
            ...
            for fixture_ptr in B2body::get_fixture_list(&RefCell::borrow(&body_ptr)).iter() {
                ...
                match shape.as_chain() {
                    Some(chain) => {
                        let points: Vec<Vector2I> = chain.m_vertices.iter().map(|v| {
                            camera.transform.xform_inverse(b2vec_to_vector2f(body.get_world_point(*v))).rounded()
                        }).collect();
                ...
                match shape.as_polygon() {
                    Some(polygon) => {
                        let points: Vec<Vector2I> = polygon.m_vertices.iter().map(|v| {
                            camera.transform.xform_inverse(b2vec_to_vector2f(body.get_world_point(*v))).rounded()
                        }).collect();
                        renderer::draw_line_loop(&mut canvas, points[0..polygon.m_count].to_vec(), Some(color))
                    }
                    None => ()
                }
            }
